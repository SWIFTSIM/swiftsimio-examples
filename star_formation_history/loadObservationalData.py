...
def read_obs_data(path="observational_data"):
    ...
    obs6_a = 1. / (1+z)
    obs6_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs6_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    obs6_rho_err[0] = -obs6_rho_err[0]  + obs6_rho
    obs6_rho_err[1] = -obs6_rho + obs6_rho_err[1]

    output.append(
        ObservationalData(
            obs6_a, obs6_rho, obs6_rho_err, "Magnelli et al. (2013) [IR]"
        )
    )
    ...
    obs7_a = 1. / (1+z)
    obs7_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs7_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    ...
    obs9_a = 1. / (1+z)
    obs9_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs9_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    obs9_rho_err[0] = -obs9_rho_err[0]  + obs9_rho
    obs9_rho_err[1] = -obs9_rho + obs9_rho_err[1]

    output.append(
        ObservationalData(
            obs9_a, obs9_rho, obs9_rho_err, "Schenker et al. (2013) [UV]"
        )
    )
    ...
    obs10_a = 1. / (1+z)
    obs10_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs10_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    ...
    # sfr observational data from Bouwens 2015 
    z, rhostar, err_p, err_m = loadtxt(f"{path}/sfr_bouwens2015_no_dustcorr.dat", unpack=True)
    obs11_a = 1. / (1+z)
    obs11_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs11_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    ...
    obs12_a = 1. / (1+z)
    obs12_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs12_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    obs12_rho_err[0] = -obs12_rho_err[0]  + obs12_rho
    obs12_rho_err[1] = -obs12_rho + obs12_rho_err[1]

    output.append(
        ObservationalData(
            obs12_a, obs12_rho, obs12_rho_err, "Oesch et al. (2018) [UV]"
        )
    )

    # sfr observational data from Finkelstein 2015 
    z, rhostar, err_p, err_m = loadtxt(f"{path}/sfr_finkelstein2015_dust.dat", unpack=True)
    obs13_a = 1. / (1+z)
    obs13_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs13_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    ...
    # sfr observational data from Finkelstein 2015 
    z, rhostar, err_p, err_m = loadtxt(f"{path}/sfr_finkelstein2015_no_dustcorr.dat", unpack=True)
    obs14_a = 1. / (1+z)
    obs14_rho = 10**rhostar / 1.65 * 0.6777/0.7 # convert to Chabrier IMF from Salpeter
    obs14_rho_err = 10**array([rhostar + (err_m), rhostar + err_p]) / 1.65 * 0.6777/0.7
    ...
    output.append(
        ObservationalData(obs15_a, obs15_rho, None, "Madau & Dickinson (2014) [$h=0.7$]")
    )
