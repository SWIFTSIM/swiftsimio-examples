...
def get_animator(args, density_bounds, temperature_bounds, bins):
    """
    ...
    def grab_metadata(n):
        ...
        data = SWIFTMetadata(filename, units)
    ...
    snapshots = list(range(args["initial"], args["final"] + 1))

    histograms = p_map(grab_data, snapshots)
    metadata = p_map(grab_metadata, snapshots)
